\documentclass[12pt,german]{scrartcl}

% See geometry.pdf to learn the layout options.
\usepackage[bottom=3.5cm,left=2.5cm,right=2.5cm]{geometry}
\geometry{a4paper}

% Use utf-8 encoding for foreign characters
\usepackage[utf8]{inputenc}

% Running Headers and footers
\usepackage{fancyhdr}
\pagestyle{fancyplain}

% To begin paragraphs with an empty line
\usepackage[parfill]{parskip}

% use multicol package
\usepackage{multicol}

% use multirow package
\usepackage{multirow}


% URLs and Links
\usepackage{url}
\usepackage{makeidx}
\usepackage[colorlinks=true, linkcolor=blue, urlcolor=blue]{hyperref}

% Surround parts of graphics with box
\usepackage{boxedminipage}

% Package for including code in the document
\usepackage{listings}
\usepackage{color}
\usepackage{moreverb}

% Math packages
\usepackage{amsmath}

% Alternative monospaced font
\usepackage[T1]{fontenc}
\usepackage[scaled]{beramono}       % Debian: texlive-fonts-extra

\usepackage{graphicx}

% Listings color
\definecolor{light-gray}{gray}{0.95}

\lstset{
    language={[x86masm]Assembler},      % choose the language of the code
    basicstyle=\ttfamily\footnotesize,  % size of fonts used for the code
    %numbers=left,                      % where to put the line-numbers
    numberstyle=\footnotesize,          % size of fonts for used line-numbers
    stepnumber=1,                       % step between line-numbers
    numbersep=10pt,                     % how far the line-numbers are from the code
    backgroundcolor=\color{light-gray}, % choose background color. You must add \usepackage{color}
    showspaces=false,                   % spaces adding particular underscores
    showstringspaces=false,             % underline spaces within strings
    showtabs=false,                     % tabs within strings adding particular underscores
    frame=single,                       % adds a frame around the code
    tabsize=4,                          % sets default tabsize to 2 spaces
    captionpos=b,                       % sets the caption-position to bottom
    breaklines=true,                    % sets automatic line breaking
    breakatwhitespace=false,            % if automatic breaks should only happen at whitespace
    escapeinside={\%*}{*)},             % if you want to add a comment within your code
    frameround=tttt,
    extendedchars=true,
    literate=%
        {Ö}{{\"O}}1
        {Ä}{{\"A}}1
        {Ü}{{\"U}}1
        {ß}{{\ss}}2
        {ü}{{\"u}}1
        {ä}{{\"a}}1
        {ö}{{\"o}}1
        {°}{{}}0
}

\makeindex

\title{Intel x86 Assembler \\ \vspace{1cm} \small{Proseminar Programmiersprachen} \\ \vspace{1cm}}
\author{Sebastian Raitza, Nico von Geyso}

\begin{document}

\vspace{5cm}

\maketitle

%\input{sections/abstract.tex}

\clearpage
\tableofcontents

\input{sections/introduction.tex}

\input{sections/history.tex}

\input{sections/syntax.tex}

\input{sections/architecture.tex}

\input{sections/instructions.tex}

\input{sections/samplecode.tex}

\input{sections/reverse_engineering.tex}

\input{sections/conclusion.tex}

\input{sections/sources.tex}

\end{document}

